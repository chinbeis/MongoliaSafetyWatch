export default function DataSourcesPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-20 font-medium">
      <h1 className="text-3xl font-extrabold text-slate-50 mb-6">Мэдээллийн эх сурвалж</h1>
      ...
      <div className="space-y-8">
        {sources.map((source) => (
          <div key={source.name} className="surface-card rounded-[2rem] p-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <h2 className="text-xl font-extrabold text-slate-50">{source.name}</h2>
              <a
                href={source.url}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-teal-300 font-bold hover:text-teal-200 transition-colors"
              >
                Албан ёсны вэбсайт
              </a>
            </div>
            <p className="text-slate-400 mb-8">{source.description}</p>
      ...
      <div className="mt-16 p-8 surface-card rounded-[2rem]">
